...
add_action( 'wp_insert_comment', function ( $id, $comment ) {
    ...
    if( $approved && $comment_type == 'comment' ) {
		...
		if( wp_miniprogram_option('bd_appkey') && wp_miniprogram_option('bd_secret') ) {
			bd_miniprogram_comment_reply_message( $comment );
		}
    }
}, 10, 2 );
...
function bd_miniprogram_comment_reply_message( $comment ) {
	...
	$page = apply_filters( 'bd_notice_reply_comment_page', $page, $post_id );
	if( $parent_id != 0 ) {
...
function we_miniprogram_comment_audit_message( $comment_id ) {
    ...
    $page = apply_filters( 'wx_notice_audit_comment_page', $page, $post_id );
    $data = array(
        "phrase1"	=> array( "value" => '评论已通过' ),
		"thing2"	=> array( "value" => html_entity_decode( wp_strip_all_tags( $thingData ) ) ),
		"date4"	    => array( "value" => $date )
    );
    $data = apply_filters( 'wx_notice_audit_comment_data', $data, $comment_id );
	$contents = array(
		"touser"		=> $touser,
		"page"			=> $page,
...
function we_miniprogram_comments_reply_action( $comment_id ) {
	...
	$page = apply_filters( 'wx_notice_reply_comments_page', $page, $post_id );
	$parents = get_comment( $parent_id );
    $parents_user_id = $parents->user_id;
	...
	$data = array(
        ...
        "name2"	=> array( "value" => wp_trim_words( $parents_content, 16, '...' ) ),
        "thing3"	=> array( "value" => wp_trim_words( $reply_content, 16, '...') ),
        "date4"	    => array( "value" => $reply_date )
    );
    $data = apply_filters( 'wx_notice_reply_comments_data', $data, $comment_id );
	$contents = array(
        "touser"		=> $touser,
        "page"			=> $page,
        "template_id"	=> $template_id,
...
function qq_miniprogram_comment_audit_message( $comment_id ) {
    ...
    $page = apply_filters( 'qq_notice_audit_comment_page', $page, $post_id );
    $data = array( );
    $data['keyword1'] = array( "value" => '评论审核通过' );
    $data['keyword2'] = array( "value" => "您发表的评论已审核" );
    $data['keyword3'] = array( "value" => current_time( 'mysql' ) );
    $data = apply_filters( 'qq_notice_audit_comment_data', $data, $comment_id );
	$contents = array(
		"touser"		=> $touser,
		"page"			=> $page,
		"template_id"	=> $template_id,
...
function qq_miniprogram_comments_reply_action( $comment_id ) {
    ...
    $page = apply_filters( 'qq_notice_reply_comments_page', $page, $post_id );
	if( $parent_id != 0 ) {
		$parents = get_comment( $parent_id );
        $parents_user_id = $parents->user_id;
        $parents_user = get_user_by( 'ID', $parents_user_id );
        ...
        $data = apply_filters( 'qq_notice_reply_comments_data', $data, $comment_id );
		$contents = array(
            "touser"		=> $touser,
            "page"			=> $page,
            "template_id"	=> $template_id,
...
function we_miniprogram_posts_update_notice( $post_id ) {
    ...
    $page = "/pages/detail/detail?id=".$post_id;
    $page = apply_filters( 'wx_notice_update_post_page', $page, $post_id );
    ...
    $data = apply_filters( 'wx_notice_update_post_data', $data, $post_id );
...
function qq_miniprogram_posts_update_notice( $post_id ) {
    ...
    $page = "/pages/detail/detail?id=".$post_id;
    $page = apply_filters( 'qq_notice_update_post_page', $page, $post_id );
    ...
    $data = apply_filters( 'qq_notice_update_post_data', $data, $post_id );
    foreach( $subscriber as $subscribe ) {
        $openid = $subscribe->openid;
        if( !$openid ) {
            continue;
