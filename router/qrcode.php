...
class WP_REST_Qrcode_Router extends WP_REST_Controller {
	...
	public function wp_post_qrcode( $request ) {
		...
		$post_path = apply_filters( 'wanzi_qrcode_path', $post_path, $post_id );
		...
		$cover_link = wanzi_post_thumbnail( $post_id );
		if( empty($cover_link) ) {
				...
				if( !empty($access_token) ) {
					...
					if( !empty( $content ) ) {
						...
						if( is_file($qrcode) ) {
							...
							$result["qrcode"]		= apply_filters( 'wanzi_qrcode_link', $qrcode_link );
							$result["cover"] 		= wanzi_cover_for_qrcode_posters( $cover_link );
						} else {
							$result["status"]		= 400; 
							$result["code"]			= "error";
